...
class LockAppDelegate: NSObject, NSApplicationDelegate {
    ...
    var menuBarOverlay: NSWindow!
    var buttonOverlay: NSWindow!
    ...
    func createMenuBarOverlay() {
        guard let screen = NSScreen.main else { return }
        let height: CGFloat = 24
        let frame = NSRect(x: 0, y: screen.frame.height - height, width: screen.frame.width, height: height)

        menuBarOverlay = NSWindow(contentRect: frame,
                                  styleMask: .borderless,
                                  backing: .buffered,
                                  defer: false)
        menuBarOverlay.isOpaque = false
        menuBarOverlay.backgroundColor = .clear
        menuBarOverlay.ignoresMouseEvents = false
        menuBarOverlay.level = .mainMenu + 2
        menuBarOverlay.makeKeyAndOrderFront(nil)
    }

    func createButtonOverlay() {
        let buttonWidth: CGFloat = 80
        let buttonHeight: CGFloat = 30
        guard let screen = NSScreen.main else { return }

        let frame = NSRect(x: 0, y: screen.frame.height - buttonHeight - 30,
                           width: buttonWidth, height: buttonHeight)

        buttonOverlay = NSWindow(contentRect: frame,
                                 styleMask: .borderless,
                                 backing: .buffered,
                                 defer: false)
        buttonOverlay.isOpaque = false
        buttonOverlay.backgroundColor = .clear
        buttonOverlay.ignoresMouseEvents = false
        buttonOverlay.level = .mainMenu + 2
        buttonOverlay.makeKeyAndOrderFront(nil)
    }
    ...
    @objc func tryToUnlock(_ sender: NSButton) {
        ...
        if passwordField.stringValue == correctPassword {
            ...
            menuBarOverlay.orderOut(nil)
            buttonOverlay.orderOut(nil)
