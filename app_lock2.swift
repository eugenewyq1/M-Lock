...
class LockAppDelegate: NSObject, NSApplicationDelegate {
    ...
    func createOverlayBlockers() {
        guard let screen = NSScreen.main else { return }
        let blockerSize = NSMakeRect(10, screen.frame.height - 40, 100, 30)

        blockerWindow = NSWindow(contentRect: blockerSize,
                                 styleMask: .borderless,
                                 backing: .buffered,
                                 defer: false)
        blockerWindow.isOpaque = false
        blockerWindow.backgroundColor = .clear
        blockerWindow.level = .mainMenu + 2
        blockerWindow.ignoresMouseEvents = false
        blockerWindow.hasShadow = false
        blockerWindow.alphaValue = 0.01 // invisible

        let blockerView = NSView(frame: blockerSize)
        blockerView.wantsLayer = true
        blockerView.layer?.backgroundColor = NSColor.black.cgColor
        blockerWindow.contentView = blockerView
        blockerWindow.makeKeyAndOrderFront(nil)
    }
    ...
    func startMonitoring() {
        monitorTimer = Timer.scheduledTimer(withTimeInterval: 2.0, repeats: true) { _ in
            ...
            self.maximizeAgentWindow()
            self.blockerWindow.orderFrontRegardless()
            let isRunning = NSWorkspace.shared.runningApplications.contains {
